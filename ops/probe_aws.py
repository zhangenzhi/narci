    ...
    if os.path.isdir(root):
        for base, _, files in os.walk(root):
            for fn in files:
                ...
                ex, mkt = vm(rel); sym = m.group(1).upper(); k = (ex, mkt, sym)
...
print(json.dumps({"freshness": freshness(RB + "/realtime"), "wal": wal(RB + "/wal")}))
'''


def _remote_script(cfg: config.FleetConfig) -> str:
...
def probe_fleet(name: str) -> dict:
    ...
    try:
        ...
        sections = fleet.split_sections(_ssm_batch(cfg))
