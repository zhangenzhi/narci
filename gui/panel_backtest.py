...
def render(base_path, history_dir, realtime_dir):
    ...
    if st.button("🚀 启动双向合约深度穿透回测", type="primary", use_container_width=True, key="bt_run_btn"):
        ...
        m3.metric("总交易次数", f"{len(df_trades)} 笔")
        m4.metric("累计消耗手续费", f"${df_trades['fee'].sum():,.2f}" if not df_trades.empty else "$0.00", delta_color="inverse")
            
        st.markdown("#### 📉 资金净值曲线与交易分布")
        fig = make_subplots(
            rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.1, 
            subplot_titles=("💰 账户总权益 (Total Equity)", "🤝 交易买卖离散点分布"),
            row_heights=[0.7, 0.3]
        )
        ...
        if not df_trades.empty:
            ...
            fig.add_trace(go.Scattergl(x=buys['datetime'], y=buys['price'], mode='markers', name='Buy', marker=dict(color='#00FF00', symbol='triangle-up', size=8)), row=2, col=1)
            fig.add_trace(go.Scattergl(x=sells['datetime'], y=sells['price'], mode='markers', name='Sell', marker=dict(color='#FF4B4B', symbol='triangle-down', size=8)), row=2, col=1)
            
        fig.update_layout(template="plotly_dark", height=600, hovermode="x unified", margin=dict(l=0, r=0, t=40, b=0))
