...
#### A. Diag 反驳 nyx "40% ts overlap" 假说(`scripts/research/diag_segreplay_vs_raw.py`)
...
builder `scripts/research/build_v10_cc_btc_cache.py`(commit `35ee282`)跑 54.6 min wall。
...
风险?** narci 拆出 4 个 gap,其中 Gap 2 用 `scripts/research/diag_v10_trade_rate_bucket.py`
跑出**实测数字**。
...
cache build 57.7 min wall(`scripts/research/build_v10_bj_btc_cache.py`,commit `80ba34d`)。
...
- **2026-05-24** (深夜) — v10 CC BTC fill-PnL cache READY(n_train=416,711 / 67 MB,
  ...
  train 直接加。Diag script:`scripts/research/diag_v10_trade_rate_bucket.py`。
...
- **2026-05-17** (晚 - segreplay fix) — 找到真 narci bug:`segmented_replay.py:153`
  ...
  + `scripts/research/diag_segreplay_vs_raw.py` diag script。4 天 verify cache 与 raw
