...
### 4.1 [P1] nyx 新 LGB binding 必须写 `model_output_unit: "bps"`

narci `33a7c31` 已修 LGB scale bug,但**只在 manifest 显式声明 `"bps"` 时
生效**。当前未 retrofit 的 nyx binding:

| nyx binding | 期望 manifest |
|---|---|
| `v4burst_v6` (36 cols) | 加 `"model_output_unit": "bps"` |
| `v4w_v6_repinned` (35 cols) | 加 `"model_output_unit": "bps"` |
| 老 OLS bindings | **不动**(默认 `"log_return"` 即正确) |
| 未来 LGB / GRU bindings | 训练 y 经 ×1e4 scale 的 → `"bps"`;raw log-return 训练的 → `"log_return"`(默认) |

不加 narci 仍然 ×10000 = 老 bug 复发,backtest threshold filter 失效。
...
### 4.3 [P2] nyx feature cache 与 narci v6 schema 对账
...
### 4.4 [P2] backtest sweep extension 协作
...
- **2026-05-16** (本次 commit) — initial cut。响应 nyx 提议
